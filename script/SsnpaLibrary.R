...
my_cluster_samples <- function(features, meta_data_df, n_pc, cluster_resolution, cluster_ref_samples, ref_samples, seed_value = 1) {
  ...
  s_obj <- Seurat::ScaleData(s_obj, verbose=F)
...
extractCellTypeStrainData <- function(cell_type, strain, sex = 'Both'){
    ...
    isPicked <- grepl(cell_type, cell_types_one_type, fixed = TRUE) & grepl(strain, strain_one_type, fixed = TRUE)
    if(strain == 'NZO'){
        isPicked <- isPicked & !(colData(cell_type_sObj)$Sample == 'MS20015' & (colData(cell_type_sObj)$Library == 'Pool-7')) ##remove this NZO male mouse due since they r not getting t2d
    ...
    labels[labels=='LF'] <- 0##control
    labels[labels=='HF'] <- 1##test or HFHS
...
cell_type_sObj=readRDS('DataPool/Cube/Parveen_data/Cube_scRNA-seq_Data_Islets_Cluster_2021-11-10.rds')

geneRef <- rtracklayer::import('DataPool/gencode.vM23.primary_assembly.annotation.gtf.gz')
